...
def correlacion_pearson(video):
    ...
    print('Tiempo Correlación de Pearson')
...
def correlacion_kendall(video):
    ...
    print('Tiempo Correlación de Kendall')
    print(time.time() - start_time)
    plt.show()
...
def correlacion_spearman(video):
    ...
    print('Tiempo Correlación de Spearman')
    print(time.time() - start_time)
    plt.show()
...
def venta_genero_7g(video):
    ...
    plt.title('Venta por géneros 7ma G')
    plt.ylabel('Ventas')
    plt.show()
...
def juego_mas_popular_10(video):
    video7th = video[(video['Platform'] == 'WiiU') | (video['Platform'] == 'PS3') | (video['Platform'] == 'X360')]
    fig = video7th.sort_values('Global_Sales', ascending=False)[:10]
    ...
    fig.layout.title = "Top juegos más populares 7G"
    plotly.offline.plot(fig)
    plt.show(fig)

def venta_anual_genero(video):
    video7th = video[(video['Platform'] == 'WiiU') | (video['Platform'] == 'PS3') | (video['Platform'] == 'X360')]
    fig = (video7th.pivot_table(index=['Year_of_Release'], values=['Global_Sales'], columns=['Genre'], aggfunc=np.sum, dropna=False,)['Global_Sales']
        .iplot(subplots=True, subplot_titles=True, asFigure=True, fill=True,title='Ventas anuales por género 7G'))
...
def venta_anual_consolas_region(video):
    video7th = video[(video['Platform'] == 'WiiU') | (video['Platform'] == 'PS3') | (video['Platform'] == 'X360')]
    fig = video7th.pivot_table(index=['Platform'], values=['NA_Sales','EU_Sales','JP_Sales','Other_Sales'],                       aggfunc=np.sum, dropna=False,).iplot( asFigure=True,xTitle='Consola',yTitle='Ventas en millones',title='Venta de juegos por consola y por región 7G')
    plotly.offline.plot(fig)
...
def definir_hits(dataset):
    ...
    plt.title('Matriz Confusión de Conj. Entrenamiento')
    plt.show()
...
def analisis_critic_score(dataset):
    video7th = dataset[(dataset['Platform'] == 'WiiU') | (dataset['Platform'] == 'PS3') | (dataset['Platform'] == 'X360')]
    dfh = video7th.dropna(subset=['Critic_Score']).reset_index(drop=True)
...
root = Tk()
root.title("Análisis Consolas 7ma generación")
...
feet_entry1 = ttk.Label(mainframe, text="Gráficos Dinámicos:", width=7)
feet_entry1.grid(column=1, row=4, sticky=(W, E))
ttk.Button(mainframe, text="Top juegos mas populares", command=lambda: juego_mas_popular_10(video)).grid(column=1, row=5, sticky=W)
ttk.Button(mainframe, text="Ventas anuales por género", command=lambda: venta_anual_genero(video)).grid(column=2, row=5, sticky=W)
ttk.Button(mainframe, text="Venta de juegos por consola y por región", command=lambda: venta_anual_consolas_region(video)).grid(column=3, row=5, sticky=W)

feet_entry2 = ttk.Label(mainframe, text="Análisis:", width=7)
...
ttk.Button(mainframe, text="Critic Score vs Global Sales", command=lambda: critic_vs_global(video)).grid(column=3, row=7, sticky=W)

feet_entry3 = ttk.Label(mainframe, text="Correlación:", width=7)
...
feet_entry4 = ttk.Label(mainframe, text="Predicción:", width=7)
...
#feet_entry.grid(column=1, row=14, sticky=(W, E))


feet_entry5 = ttk.Label(mainframe, text="Análisis visual:", width=7)
feet_entry5.grid(column=1, row=15, sticky=(W, E))
ttk.Button(mainframe, text="Agrupación Jerárquica", command=lambda: primer_mapa_visual()).grid(column=2, row=16, sticky=W)
ttk.Button(mainframe, text="Árbol Ordenado Radial", command=lambda: segundo_mapa_visual()).grid(column=3, row=16, sticky=W)
